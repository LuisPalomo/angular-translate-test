...
import {
  ...
  getDefaultLang,
  getLangs
} from './translate/reducers/translate';
...
export class PruebaService {
     ...
     */
    constructor(
      public store: Store<TranslateState>,
      ...
      if (this.isolate) {
        const translations$ = this.store.select(getTranslations);
        const currentLang$ = this.store.select(getCurrentLang);
        const defaultLang$ = this.store.select(getDefaultLang);
        const langs$ = this.store.select(getLangs);

        translations$
          .withLatestFrom(currentLang$)
          .subscribe(([translations, lang]) => {
            this._onTranslationChange.emit({translations, lang});
          });
